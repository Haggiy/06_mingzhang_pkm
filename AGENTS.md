...
- 后续开发 iOS UI 功能时，默认同时补充自动化验证；如果暂时无法自动化，交付说明中必须写明原因和替代验证方式。
- 原生 iOS UI 自动化默认使用 XCTest / XCUITest + iOS Simulator，通过 `xcodebuild test` 在指定模拟器上运行。
- SwiftUI / UIKit 中的关键可交互控件、页面状态和断言节点必须配置稳定的 `accessibilityIdentifier`；UI 测试优先通过 identifier 定位元素，不依赖易变文案。
- App 应支持 UI 测试启动模式，例如通过 launch argument 或环境变量 `UITesting` 注入固定样例数据、清理本地状态、关闭动画，并隔离网络、时间等不确定因素。
- UI 测试覆盖用户主路径和关键验收场景，例如新建/编辑流水、账月选择、导入预览、分类确认、结果汇总等。
- 复式记账、账月结转、投资平均成本、导入分类等核心账务规则优先由单元测试或集成测试覆盖；UI 测试只验证用户路径和界面集成结果。
- 对外观稳定性要求高的页面可以补充 Snapshot Test；Snapshot Test 不替代流程型 XCUITest。
- 新增或修改 PRD、Spec、页面低保真或开发输入文档时，如果涉及可交互 UI，应同步补充自动化验收点或标注暂不自动化原因。
